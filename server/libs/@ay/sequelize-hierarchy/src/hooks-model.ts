...
export default function hooksModelFn(sequelize = Sequelize, patches) {
  const { co, coAll } = makeCo();
  ...
  function* _beforeUpdate(item, options) {
    ...
    if (oldParentId === undefined || oldLevel === undefined) {
      const itemRecord = yield findOne(
        model,
        addOptions(
          {
    ...
    if (level !== oldLevel) {
      ...
      sql = replaceFieldNames(
        sql,
        { level: levelFieldName, id: primaryKey },
        model,
      );
      sql = replaceFieldNames(
        sql,
      ...
      yield query(
        ...
        sql,
        addOptions(
          { replacements: { id: itemId, levelChange: level - oldLevel } },
          options,
        ),
      );
    }
