...
import hooksModelFn from './hooks-model';
import { addOptions, humanize, makeCo } from './utils';

// Exports

export default function modelExtendsFn(Sequelize, patches) {
  const {
      findAll,
      underscoredIf,
      uppercaseFirst,
  ...
  function isHierarchy(options) {
    ...
    options = Object.assign(
      {
        ...
        levelFieldType: ['postgres', 'mssql'].includes(
    ...
    _.defaults(options, {
      foreignKey: underscoredIf(
        `${as}${uppercaseFirst(primaryKey)}`,
        underscored,
      ),
      throughKey: underscoredIf(
        `${modelName}${uppercaseFirst(primaryKey)}`,
        underscored,
      ),
      throughForeignKey: underscoredIf(
        `${singularize(ancestorsAs)}${uppercaseFirst(primaryKey)}`,
        underscored,
      ),
    ...
    if (throughTable === undefined) {
      if (options.freezeTableName) {
        ...
        const ancestorsAsCamel =
          camelThrough || underscoredAll
            ? uppercaseFirst(ancestorsAs)
            : ancestorsAs;

        throughTable = underscoredIf(
          `${pluralize(modelName)}${ancestorsAsCamel}`,
          underscoredAll,
        );
    ...
    if (onDelete == null) {
      ...
      if (!['RESTRICT', 'CASCADE'].includes(onDelete)) {
        throw new Sequelize.HierarchyError(
          "onDelete on hierarchies must be either 'RESTRICT' or 'CASCADE'",
        );
      }
    }
    ...
    const primaryKeyType = modelAttributes[primaryKey].type,
      { throughKey, throughForeignKey, throughUniqueKey } = options;
    const throughFields = {};
  ...
  function* rebuildHierarchy(options) {
    ...
    const processLevel = co(function* (level, parents) {
      ...
      const where = {
        [foreignKey]: parents ? parents.map((item) => item.id) : null,
      };
      ...
      items = items.map((item) => {
        ...
        const parentPath = parents.find(
          (thisItem) => thisItem.id === parentId,
        ).path;
