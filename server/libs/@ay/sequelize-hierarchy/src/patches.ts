import * as semverSelect from 'semver-select';
...
export function patchesFn(Sequelize) {
  ...
  return semverSelect.object(sequelizeVersion, {
     ...
     */
    query: {
      '^2.0.0': (sequelize, sql, options) =>
        sequelize.query(sql, null, options),
      '>=3.0.0': (sequelize, sql, options) => sequelize.query(sql, options),
    },

    findOne: {
      '^2.0.0': (model, options) =>
        model.find(options, addOptions({}, options)),
    ...
    findAll: {
      '^2.0.0': (model, options) =>
        model.findAll(options, addOptions({}, options)),
