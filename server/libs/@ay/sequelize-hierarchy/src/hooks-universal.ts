...
export default function hooksUniversalFn(sequelize = Sequelize, patches) {
    ...
    _.forIn(attributes(model), (field, fieldName) => {
      ...
      if (hierarchy) {
        throw new HierarchyError(
          `You cannot define hierarchy on two attributes, or an attribute and the model options, in '${model.name}'`,
        );
      }
  ...
  function beforeFindAfterExpandIncludeAll(options) {
    ...
    if (options.hierarchy) {
      if (!model.hierarchy) {
        throw new HierarchyError(
          `You cannot get hierarchy of '${model.name}' - it is not hierarchical`,
        );
      }
    ...
    for (const include of options.include) {
      ...
      if (include.hierarchy) {
        if (!includeModel.hierarchy) {
          throw new HierarchyError(
            `You cannot get hierarchy of '${includeModel.name}' - it is not hierarchical`,
          );
        }
      ...
      // Check includes
      hierarchyExists =
        hierarchyExists || checkHierarchy(include, includeModel);
    }
  ...
  function convertHierarchy(results, model, parent) {
    ...
    for (const item of results) {
      ...
      if (!thisParent) {
        throw new HierarchyError(
          `Parent ID ${thisParentId} not found in result set`,
        );
      }
