...
import { detectionComment } from '../tools/detection-comment';
...
export class Model extends Output {
  ...
  public constructor(public config: Config, public module: SharedModule) {
    super(`${config.dist}/src/${module.name}/model.ts`, config);
  ...
  public async generateContent(): Promise<string[]> {
    ...
    if (this.controller) {
      content.push(``);
      content = content.concat(...this._generateModelContent());
    }
  ...
  private _generateImportsContent() {
    ...
    return ImportType.toString(
      this.imports,
      `${this.config.dist}/src/${this.module.name}`,
    );
  }
  ...
  private _generateModelContent(): string[] {
    let content = [];
    ...
    findHasDecoratorMethods(this.controller, 'Share').map((method) => {
      ...
      const comment = detectionComment(method);
  ...
  private _processParameter(parameter: ts.ParameterDeclaration) {
